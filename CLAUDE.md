...
│   ├── CronometroInput.jsx # Campo MM:SS:CC com máscara automática (dígitos preenchem da direita pra esquerda), Desafio RJNet (D-098)
│   └── modals/
...
│       ├── DesafioTentativas.jsx  # Bloco progressivo de tentativas de 1 participante — só mostra "+ Adicionar tentativa N" após a anterior (D-098)
...
│       ├── DesafioRanking.jsx     # Top 10 pela melhor tentativa de cada participante (D-098), coluna Prêmio do participante + Prêmio por posição (D-095)
│       ├── DesafioGanhadores.jsx  # 🏆 Ganhadores Instantâneos (melhor tentativa com acerto exato, D-098) + controle de entrega de prêmio
...
| `timer_challenge_entries` | Desafio RJNet (D-089, D-090, D-098): representa só o PARTICIPANTE de um dia — nome/telefone (sem número identificador, removido em D-090), prêmio que está concorrendo/recebeu (`prize_type`, selecionável já no cadastro desde D-098), controle de entrega (`delivered`/`delivery_responsible`/`delivery_at`); resultado do cronômetro NÃO fica mais aqui — ver `timer_challenge_attempts` |
...
| `src/api/desafioApi.js` | ~175 | Factory createDesafioApi — CRUD de dias + cadastro do participante já com a 1ª tentativa + `addDesafioTentativa` (tentativa nova sobre id existente, até `maxTentativas`) + `updateDesafioParticipante` (edição rápida nome/telefone) + controle de entrega de prêmio + `saveDesafioPremio`/`saveDesafioPremiosRanking` (D-089, D-091, D-092, D-093, D-098) |
...
| `src/features/desafio/DesafioTentativas.jsx` | ~70 | Bloco progressivo de tentativas de 1 participante — lista as já registradas, "+ Adicionar tentativa N" só até `maxTentativas` (D-098) |
...
| `src/features/desafio/DesafioGanhadores.jsx` | ~100 | 🏆 Ganhadores Instantâneos (participantes cuja melhor tentativa acertou exatamente, D-098) + controle de entrega de prêmio (tipo, entregue, responsável, horário) (D-089) |
...
| `src/utils/csv.js` | ~180 | Exportação CSV de leads por evento, por mês e da fila de distribuição — `exportLeadsSemVendedorCSV` para os leads "em espera" (etapa 3, D-058, D-085) + `exportDesafioEntriesCSV` (etapa 3, D-089, D-095, coluna "Tentativas" e "Prêmio" unificado desde D-098 — melhor tentativa já resolvida pelo chamador, sem lógica de negócio no arquivo) |

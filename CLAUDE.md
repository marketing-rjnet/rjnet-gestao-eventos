...
│   ├── leads/
│   │   ├── LeadsTab.jsx      # Filtros, gráfico e exportação CSV de leads; fila de distribuição com export dos leads em espera (D-085) e listas de Eventos/Meses/Leads sem vendedor em accordion (D-086, D-087) (etapa 11)
...
│       ├── DesafioPremiosRanking.jsx # Prêmio por posição do ranking (1º-10º), independente do prêmio geral (D-092)
...
├── migracao-desafio-premio-ranking.sql # Coluna timer_challenge_events.prize_ranking (jsonb, 1 prêmio por posição 1º-10º) + RPC recriada (D-092)
...
| `timer_challenge_events` | Desafio RJNet — Acerte 00:03:33 (D-089): dias/edições do desafio — nome, slug, `target_centiseconds` (tempo-alvo, configurável, default 333), ativo, `prize_description`/`prize_image_path`/`prize_updated_at` (prêmio do dia, D-091), `prize_ranking` (jsonb, prêmio por posição do ranking 1º-10º, D-092); sem leitura `anon` (tela pública só via RPC) |
...
- Bucket `desafio-premios` (público) — imagem do prêmio por dia do Desafio, path `<event_id>.<ext>` (D-091). Escrita restrita a `marketing` via policies em `storage.objects`, mesmo padrão do bucket `ofertas`.
...
| Leads | marketing, comercial | Export CSV por evento e por mês de referência (D-058), export CSV dos leads em espera na fila de distribuição (D-085), listas de Eventos/Meses e Leads sem vendedor em accordion fechado por padrão (D-086, D-087), auditoria de exportação (D-059: comercial edita/exclui leads de qualquer vendedor) |
...
| Desafio | marketing | **Desafio RJNet — Acerte 00:03:33** (D-089, D-090): cadastro de participantes por dia/edição do desafio (nome/telefone/resultado do cronômetro, MM:SS:CC — sem "número do participante", D-090), ranking Top 10 por menor diferença (nunca inclui acertos exatos), lista de 🏆 Ganhadores Instantâneos com controle de entrega de prêmio, sub-aba **Prêmio** com descrição + imagem opcional do prêmio do dia (streamings — Disney+/HBO Max/RJNet Play, D-091) e prêmios individuais por posição do ranking 1º-10º (nome + ícone pequeno cada, D-092), painel de estatísticas + export CSV, e QR Code/link de uma tela pública de TV (`/tv/:slug`) com ranking + KPIs (participantes, ganhadores, menor diferença, média dos tempos, alvo) em tempo real, painel de Prêmio ao lado do ranking (D-091), coluna própria de prêmio por posição na tabela do ranking (D-092) e animação de novo ganhador; múltiplos dias 100% independentes; marketing-only (não `comercial`) |
...
| `src/api/desafioApi.js` | ~143 | Factory createDesafioApi — CRUD de dias + cadastro/edição de participações (calcula resultado antes de gravar) + controle de entrega de prêmio + `saveDesafioPremio`/`saveDesafioPremiosRanking` (D-089, D-091, D-092) |
| `src/hooks/useDesafioPainelPublico.js` | ~98 | Hook da tela de TV — RPC pública + Broadcast em modo Supabase, poll leve em modo local; detecta ganhador novo comparando com a última lista vista; espelha o prêmio do dia no `painelLocal()` (D-089, D-091) |
| `src/public/DesafioPublico.jsx` | ~206 | Tela pública de TV (`/tv/:slug`) — sem sessão, tela cheia, ranking Top 10 (com coluna própria de prêmio por posição, D-092) + painel de Prêmio geral (quando configurado) + ganhadores instantâneos + animação "Novo Ganhador" (confete, ~5s, volta sozinha) (D-089, D-091, D-092) |
| `src/features/desafio/DesafioTab.jsx` | ~110 | Lista/criação de dias do desafio (tempo-alvo configurável por dia) (D-089) |
| `src/features/desafio/DesafioDetail.jsx` | ~62 | Sub-navegação de um dia (Cadastro/Ranking/Ganhadores/Prêmio/Painel/Tela de TV); carrega participações on-demand (D-089, D-091, D-092) |
...
| `src/features/desafio/DesafioPremio.jsx` | ~90 | Prêmio do dia: descrição (até 500 caracteres) + upload de imagem opcional (bucket `desafio-premios`), preview local via blob URL (D-091) |
| `src/features/desafio/DesafioPremiosRanking.jsx` | ~110 | Prêmios por posição do ranking (1º ao 10º), independente do prêmio geral do dia: 10 linhas com nome + ícone opcional pequeno cada, 1 botão salva tudo (D-092) |
...
| `supabase/migracao-desafio-premio.sql` | ~120 | Colunas `prize_description`/`prize_image_path`/`prize_updated_at` em `timer_challenge_events`, bucket `desafio-premios` + policies, RPC pública recriada com o prêmio no payload (D-091) |
| `supabase/migracao-desafio-premio-ranking.sql` | ~110 | Coluna `prize_ranking` (jsonb, array de até 10 posições) em `timer_challenge_events`, RPC pública recriada de novo com `prizeRanking` no payload (D-092) |
...
| `src/features/leads/LeadsTab.jsx` | ~607 | Exportação de leads por evento/mês; `FilaDistribuicao` (leads sem vendedor de QR Code/Formulário/Simulador) com export CSV dos leads em espera (D-085); listas de Eventos/Meses/Leads sem vendedor em accordion fechado por padrão, mesmo padrão visual de `MesDetail.jsx` (D-086, D-087); `DemandaPorRegiao` (relatório interno por cidade/bairro) (etapa 11, D-058, D-085, D-086, D-087) |

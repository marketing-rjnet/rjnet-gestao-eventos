...
| `src/api/desafioApi.js` | ~200 | Factory createDesafioApi — CRUD de dias + cadastro do participante já com a 1ª tentativa (nome/telefone/prêmio/já-cliente) + `addDesafioTentativa` (tentativa nova sobre id existente, até `maxTentativas`) + `updateDesafioTentativa` (corrige o valor de uma tentativa já registrada, sem criar uma nova, D-100) + `updateDesafioParticipante` (edição rápida nome/telefone/já-cliente) + controle de entrega de prêmio + `saveDesafioPremio`/`saveDesafioPremiosRanking` — ambas com broadcast no `onSuccess` (D-099: faltava, Tela de TV não atualizava em modo Supabase) (D-089, D-091, D-092, D-093, D-098, D-099, D-100) |
...
| `src/features/desafio/DesafioPremiosRanking.jsx` | ~85 | Prêmios por posição do ranking (1º ao 10º), independente do prêmio geral do dia: cada posição escolhe uma de 4 opções fixas (RJNET Móvel/HBO Max/Disney+/RJNET Play) via botões, sem imagem, 1 botão salva tudo (D-092, D-093, D-102) |

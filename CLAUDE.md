...
| `src/lib/desafioCronometro.js` | ~82 | Domínio puro do Desafio RJNet — parse/format MM:SS:CC↔centésimos, `calcularResultadoDesafio()` (diferença + acerto exato), catálogos `TIPOS_PREMIO`/`PREMIOS_POSICAO_RANKING`; sem imports, testável standalone (D-089, D-093) |
...
| `src/api/desafioApi.js` | ~131 | Factory createDesafioApi — CRUD de dias + cadastro/edição de participações (calcula resultado antes de gravar) + controle de entrega de prêmio + `saveDesafioPremio`/`saveDesafioPremiosRanking` (D-089, D-091, D-092, D-093) |
...
| `src/public/DesafioPublico.jsx` | ~198 | Tela pública de TV (`/tv/:slug`) — sem sessão, tela cheia, ranking Top 10 (com coluna própria de prêmio por posição, texto do catálogo fixo, D-092/D-093) + painel de Prêmio geral (quando configurado) + ganhadores instantâneos + animação "Novo Ganhador" (confete, ~5s, volta sozinha) (D-089, D-091, D-092, D-093) |
...
| `src/features/desafio/DesafioPremiosRanking.jsx` | ~85 | Prêmios por posição do ranking (1º ao 10º), independente do prêmio geral do dia: cada posição escolhe uma de 3 opções fixas (RJNET Móvel/HBO Max/Disney+) via botões, sem imagem, 1 botão salva tudo (D-092, D-093) |

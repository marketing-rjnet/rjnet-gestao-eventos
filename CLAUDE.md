...
| `src/api/desafioApi.js` | ~180 | Factory createDesafioApi — CRUD de dias + cadastro do participante já com a 1ª tentativa (nome/telefone/prêmio/já-cliente) + `addDesafioTentativa` (tentativa nova sobre id existente, até `maxTentativas`) + `updateDesafioParticipante` (edição rápida nome/telefone/já-cliente) + controle de entrega de prêmio + `saveDesafioPremio`/`saveDesafioPremiosRanking` — ambas com broadcast no `onSuccess` (D-099: faltava, Tela de TV não atualizava em modo Supabase) (D-089, D-091, D-092, D-093, D-098, D-099) |

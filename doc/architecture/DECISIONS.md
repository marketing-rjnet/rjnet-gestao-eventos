...
### [D-098] — Desafio RJNet: múltiplas tentativas por participante, edição rápida, prêmio no cadastro e máscara automática do cronômetro

**Data:** 2026-08-10
**Tipo:** Feature / Evolução de módulo existente

**Contexto:** O responsável pediu 4 evoluções no módulo Desafio RJNet (D-089–D-097), preservando arquitetura/componentes/padrões existentes: (1) até 3 tentativas por participante SEM recadastrar a pessoa, com interface progressiva (nunca mostrar os 3 espaços vazios de uma vez) e cálculo automático da melhor tentativa por valor numérico; (2) máscara automática no campo de cronômetro (operador digita só números, o `:` aparece sozinho); (3) edição rápida de nome/telefone do participante já cadastrado, sem duplicar registro nem afetar tentativas/ranking; (4) prêmio vinculado ao participante, selecionável no cadastro, reaproveitando estrutura de prêmio já existente na interface. Tudo deveria refletir automaticamente no Painel TV via o mesmo mecanismo de realtime já existente (Broadcast + RPC pública), sem criar uma segunda lógica de ranking/realtime.

**Decisão:**
- **Tabela filha `timer_challenge_attempts`** (nova, `migracao-desafio-tentativas.sql`) — 1 linha por tentativa (`entry_id`, `attempt_number`, resultado/diferença/acerto exato, `event_id` denormalizado). `timer_challenge_entries` perde as colunas de resultado (migradas via backfill, a tentativa única existente vira Tentativa 1) e passa a representar só o PARTICIPANTE (nome, telefone, prêmio, entrega). Decisão explícita de **não** usar colunas `tentativa1`/`tentativa2`/`tentativa3` — não escalaria pra outro `max_attempts` nem permitiria ordenar/agregar por tentativa em SQL simples. `timer_challenge_events` ganha `max_attempts` (default 3, check 1–10) — configurável por dia, mesmo espírito de `target_centiseconds`.
- **"Melhor tentativa" — 1 única função, replicada em 2 lugares de propósito**: `melhorTentativa(tentativas)` (`src/lib/desafioCronometro.js`, sem imports) compara por `differenceCentiseconds` numérico (nunca string), empate pela de menor `attemptNumber`. A mesma regra é reescrita em SQL na RPC pública `timer_challenge_painel_publico` (CTEs `best`/`best_attempt`/`winning_attempt`) — nunca uma 3ª implementação em nenhuma tela administrativa, todas chamam a função JS.
- **Cadastro cria o participante já com a Tentativa 1** (`addDesafioEntry`); tentativas seguintes usam `addDesafioTentativa(entryId, resultado)` sobre o MESMO `id` — bloqueado além de `maxTentativas`. Interface progressiva: `DesafioTentativas.jsx` lista só as tentativas já registradas + um botão "+ Adicionar tentativa N" (nunca 3 campos vazios), usado em `DesafioCadastro.jsx` (console do operador, com lista "Participantes cadastrados" buscável, pra nunca precisar recadastrar).
- **`CronometroInput.jsx`** (novo componente, `src/components/`): máscara MM:SS:CC que preenche da DIREITA pra esquerda a partir dos dígitos digitados (`formatarDigitosCronometro()`, mesmo princípio de um campo de valor monetário) — captado via `onKeyDown` (bloqueia qualquer tecla que não seja dígito/Backspace, sem comportamento estranho no Backspace) com fallback por `onChange` nativo pra autofill/testes automatizados (`.fill()` do Playwright não passa por `onKeyDown`). Usado no cadastro, em "adicionar tentativa" e no tempo-alvo (`DesafioTab.jsx`) — reaproveitado, não duplicado por tela.
- **`updateDesafioParticipante(entryId, { participantName, phone })`**: PATCH só de nome/telefone sobre o `id` existente — nunca nome/telefone como identificador, nunca cria um 2º participante; tentativas/prêmio/entrega/ranking preservados via spread. `DesafioEditarParticipante.jsx` (modal padrão, mesmo desenho de `MaterialModal.jsx`) acessível em Cadastro, Ranking e Ganhadores.
- **Prêmio no cadastro reaproveita `prizeType`/`TIPOS_PREMIO`** (já existiam desde D-089, só editáveis em Ganhadores no momento da entrega) — passam a ser selecionáveis já no cadastro (botões, não `<select>`, pra velocidade do operador), sem coluna nem catálogo novo; segue ajustável depois na entrega. Mantido conceitualmente separado do prêmio POR POSIÇÃO do ranking (D-092/D-093/D-095, inalterado — catálogo `PREMIOS_POSICAO_RANKING`, 3 opções fixas) — são 2 prêmios distintos (do participante vs. da posição), `DesafioRanking.jsx` mostra as 2 colunas lado a lado.
- **Painel TV — zero mudança de mecanismo**: `DesafioPublico.jsx` não precisou de nenhuma alteração de código — a RPC pública mantém o mesmo contrato de payload (`participant_name`/`result_display`/`difference_centiseconds`/`prize_type`/`delivered`), só recalculado por participante (agregando as tentativas) em vez de por linha única; Broadcast (`desafioRealtime.js`) continua disparado nos mesmos pontos. Validado via Playwright: edição de nome no painel administrativo aparece na Tela de TV sem F5, via o poll/broadcast já existente.

**Alternativas Avaliadas:**
- **Colunas `tentativa1_resultado`/`tentativa2_resultado`/`tentativa3_resultado` em `timer_challenge_entries`** — descartada: exigiria uma coluna nova a cada mudança de `max_attempts`, tornaria "melhor tentativa"/ranking uma expressão SQL frágil (LEAST/CASE aninhado) e violaria a instrução explícita do responsável.
- **Reusar `TIPOS_PREMIO` vs. criar catálogo próprio de 3 prêmios pro cadastro** — avaliado usar o catálogo `PREMIOS_POSICAO_RANKING` (3 opções, sem "Outro") por bater mais literalmente com "normalmente trabalhamos com 3 prêmios"; descartado por criar 2 catálogos com nomes quase iguais mas strings diferentes (`"RJNET Móvel"` vs. `"RJNET Móvel 24GB"`) escritos no MESMO campo `prize_type` em momentos diferentes (cadastro vs. entrega), o que quebraria a seleção pré-preenchida em Ganhadores. `TIPOS_PREMIO` já é literalmente "o prêmio associado a um participante" na interface existente (Ganhadores) — reaproveitar esse é mais fiel a "não criar estrutura duplicada" do que introduzir uma modelagem nova.
- **Máscara via `onChange` tradicional (padrão `maskCpf`/`maskTel`)** — descartada pra este campo: o preenchimento é da DIREITA pra esquerda (padStart), então o texto exibido já nasce com 6 dígitos a partir do 1º caractere, impossibilitando inferir inserção/remoção só pelo comprimento do valor final (viável para CPF/telefone, que crescem da esquerda). `onKeyDown` foi a técnica escolhida, com fallback de `onChange` só pra entrada não-teclado (autofill/testes).

**Impactos:** Nenhuma quebra de comportamento existente — ganhadores instantâneos/ranking continuam sendo o mesmo universo de participantes só filtrado; RPC pública recriada preserva o payload consumido pela Tela de TV; CSV ganha coluna "Tentativas" e simplifica "Prêmio (Ganhador Instantâneo)" pra só "Prêmio" (mesmo campo, preenchido mais cedo). Ordem de deploy: `migracao-desafio-tentativas.sql` deve rodar antes do deploy do frontend (a UI já assume `tentativas`/`maxTentativas` no shape do participante).

**Arquivos Afetados:** `supabase/migracao-desafio-tentativas.sql` (novo); `src/lib/desafioCronometro.js` (`MAX_TENTATIVAS_PADRAO`, `formatarDigitosCronometro`, `melhorTentativa`); `src/components/CronometroInput.jsx` (novo); `src/features/desafio/DesafioTentativas.jsx` (novo), `DesafioEditarParticipante.jsx` (novo), `DesafioCadastro.jsx` (reescrito), `DesafioRanking.jsx`, `DesafioGanhadores.jsx`, `DesafioDashboard.jsx`, `DesafioTab.jsx`, `DesafioDetail.jsx`; `src/api/desafioApi.js` (`addDesafioTentativa`, `updateDesafioParticipante`, `addDesafioEntry`/`addDesafioEvento` atualizados); `src/lib/dataService.js` (mapeadores de tentativa, `fetchDesafioEntries` com 2 queries); `src/context/AppProvider.jsx` (wiring das novas funções); `src/hooks/useDesafioPainelPublico.js` (modo local espelha a nova agregação); `src/utils/csv.js` (coluna "Tentativas"); `tests/desafioCronometro.unit.test.js`, `tests/desafio.test.js` (cobertura das 4 evoluções).

**Riscos:** Nenhum identificado que exija mitigação adicional — mudança de schema é aditiva/idempotente com backfill; único ponto de atenção é a ordem de deploy (migração antes do frontend, mesma regra já seguida pelas migrações anteriores do módulo). Validado via `node tests/desafioCronometro.unit.test.js` (43 asserções) e `npx playwright test tests/desafio.test.js` (11 cenários E2E, incluindo Painel TV refletindo edição sem F5). `migracao-desafio-tentativas.sql` aplicada no SQL Editor do Supabase de produção pelo responsável (backfill dos participantes já cadastrados confirmado, sem perda de dado) — mergeada na `main`.
...
### [D-104] — Módulo de Landing Pages + Aquisição: infraestrutura genérica de tracking, atribuição e conversão (LP Fibra é a primeira instância)

**Data:** 2026-09-03
**Tipo:** Feature / arquitetura (novo domínio) / migração de banco / Edge Functions públicas

**Contexto:** O marketing precisa enxergar o caminho Campanha → Landing Page → Visita → Lead → WhatsApp (→ Atendimento → Venda, em fase futura). A primeira LP a conectar é a de Fibra (`fibra.rjnet.com.br`), mas o briefing exige que a arquitetura seja genérica — LP TV/Móvel/sazonais entram depois **sem alteração de código**. Análise completa dos 13 pontos (arquitetura atual, entidades reutilizáveis, riscos etc.) em `doc/aquisicao/AQUISICAO_ANALISE.md`.

**Decisão:**
- **Entidade genérica `landing_pages`** (id/nome/slug/domínio/serviço/status/campanha_padrao/WhatsApp/`tracking` jsonb). A LP Fibra é só um INSERT (seed da migração, `slug='fibra'`, `whatsapp_number=null`). Nada de `FibraService`/`fibra_leads`.
- **Sessão + eventos próprios do módulo**: `lp_sessions` (UUID gerado no cliente; UTM first-touch, referrer, URL, `device` — **sem IP nem user-agent**, minimização LGPD) e `lp_events` (UMA tabela pra todos os tipos; `nome` validado por whitelist em código — `EVENTOS_LP` em `src/lib/aquisicao.js`, espelhada na Edge Function). Taxonomia própria: `page_view`, `cta_click`, `form_start`, `form_submit`, `lead_created` (só servidor), `whatsapp_click`.
- **Lead continua sendo `leads`**: só `origem='landing_page'` + 2 colunas de vínculo (`landing_page_id`, `lp_session_id`). UTM reaproveita `leads.utm` (D-072). A UTM da **sessão** é a fonte de verdade (body é fallback). Lead nasce sem vendedor e cai na fila "Leads sem vendedor" como Form Builder/Simulador; `origemDetalhe` mostra "Landing Page — LP Fibra — utm_campaign".
- **Campanha não é tabela**: o projeto nunca teve uma (`simuladores.campanha` é agrupador texto); campanha = dimensão `utm_source/medium/campaign/content` das sessões. `landing_pages.campanha_padrao` só preenche `utm_campaign` de visita sem UTM. Evita um "sistema paralelo de campanhas".
- **Escrita pública só por Edge Function** (`service_role`), no molde de `submeter-formulario`/`submeter-simulador` e reutilizando `_shared/captacao.ts`: `rastrear-lp` (sessão+eventos; LP `ativa` obrigatória; UUID; whitelist; ≤20 eventos/req; props achatadas; teto de 500 eventos/sessão; `lead_id` só aceito se o lead existir E for da mesma LP; aceita `text/plain` pra beacon) e `submeter-lp` (honeypot, sanitização, `containsLink`, telefone, **consentimento obrigatório**, rate limit 5/10min por IP, `origem_ip` só no lead, dedupe 24h por telefone na mesma LP devolvendo o lead existente, `lead_created` gravado após o insert — falha do evento nunca desfaz o lead).
- **Leitura pública só por RPC** `landing_page_publica(slug)` (SECURITY DEFINER, `grant to anon` — padrão D-103/D-089): devolve identidade + destino do WhatsApp + IDs públicos de tracking. Tabelas sem policy `anon`; `lp_sessions`/`lp_events` sem policy de escrita pra `authenticated`.
- **Métricas por RPC** `aquisicao_metricas(...)` (SECURITY DEFINER, exige `papel_atual()='marketing'`): totais, por LP, por campanha, por dia num único jsonb — sessões/eventos nunca entram no `AppProvider`. Mesma regra replicada em `calcularFunil()` (`src/lib/aquisicao.js`, sem imports) pro modo local e pros testes unitários (uma regra, dois runtimes — mesmo princípio de `simulador.js` ↔ Deno).
- **SDK público `public/rjnet-lp.js`** = a *Tracking Layer*: vanilla, servido pelo CRM, embutido no `<head>` da LP com `data-lp/data-supabase-url/data-anon-key`. Cria a sessão (sessionStorage), captura UTM/referrer/device, despacha cada evento interno pra uma lista de **adapters** — `interno` (Edge Function, buffer + `fetch keepalive`) e `gtm` (só se `tracking.gtm_container_id` existir: injeta o container e faz `dataLayer.push({event:'rjnet_<evento>'})`). GA4/Google Ads/Meta Pixel ficam como **pontos de extensão** (`MAPA_EVENTOS_EXTERNOS`, `RJNetLP.integrations.add`) — não implementados, por decisão do briefing. Auto-bind por `data-rjnet-cta` / `form[data-rjnet-form]` / `data-rjnet-whatsapp`. **Tracking nunca bloqueia a conversão**: erros engolidos, formulário e WhatsApp funcionam sem backend.
- **WhatsApp Fase 1**: `whatsapp_enabled`/`whatsapp_number`/`whatsapp_label`/`whatsapp_mensagem` por LP; número **nasce nulo** e é editado na tela — nunca no código. Clique vira `whatsapp_click` (com `lead_id` quando a pessoa já enviou o form); com número, o SDK abre `wa.me`; sem número, o `href` da própria LP segue. Nenhuma API/webhook/inbox de WhatsApp.
- **UI**: nova tab `aquisicao` no "Mais" do `MarketingApp` (grupo próprio "Aquisição" → "Landing Pages"), marketing-only por construção (`ComercialApp` não muda). Sub-navegação interna (padrão `DesafioDetail`): Dashboard (KPIs Visitas/Leads/Conversão/WhatsApp + funil de 4 etapas + evolução diária + cards por LP), Landing Pages (lista/cadastro/detalhe com Visão geral/Eventos/Leads/Campanhas/Conversões/Integração/Configurar), Campanhas (tabela por UTM) e Conversões (leads que clicaram no WhatsApp). Filtros: período, LP, campanha, source, medium, vendedor, status do lead — todos alimentados por dados reais; **zero número fictício** (Atendimento/Venda/Receita só entram quando existirem no sistema).
- **Retenção**: `limpar_lp_tracking_expirado()` + chave `retencao_lp_sessoes_dias` (395) + job pg_cron (guardados por `if exists`); leads seguem a retenção própria (D-064). `REVOKE` aplicado (V-02).
- **Observabilidade**: sem sistema paralelo — Edge Functions logam `[rjnet:edge:lp]` com o motivo de cada rejeição; a própria `lp_events` é a trilha ("evento recebido/associado ao lead"), exposta na aba "Eventos" da LP.

**Alternativas Avaliadas:**
- **LP renderizada dentro deste app** (rota pública `/lp/:slug`, como `/f`/`/s`) — descartado: a LP Fibra vive em domínio próprio e o briefing exige que ela não dependa do carregamento do RJNET Gestão; o CRM é consumidor, não host. Um SDK de ~8 KB resolve sem acoplar.
- **Reaproveitar `formularios` (Form Builder) como "LP"** — descartado: Form Builder é catálogo de campos de um formulário, não tem sessão/eventos/UTM por visita nem configuração de WhatsApp/tracking; forçar isso deformaria as duas entidades.
- **Tabela `campanhas` com FK em sessão/lead** — descartado: criaria uma segunda estrutura de campanha (o Simulador já usa `leads.utm`), exigiria cadastro prévio de cada UTM e quebraria a atribuição de qualquer link não cadastrado. Dimensão derivada cobre 100% do que o dashboard precisa; se um dia houver orçamento/receita por campanha, aí sim vira tabela — sem retrabalho, porque a chave (`utm_campaign`) já existe.
- **Uma tabela por tipo de evento** (`lp_page_views`, `lp_whatsapp_clicks`…) — descartado explicitamente pelo briefing e pela extensibilidade (evento novo = 1 linha na whitelist).
- **Gravar IP nas sessões pra rate limit forte no tracking** — descartado por minimização: sessão é dado anônimo e a superfície de abuso é "poluir métrica", não "criar lead"; leads continuam protegidos pelo rate limit por IP de `submeter-lp`. Aceito e documentado como risco.
- **Implementar já GA4/Ads/Meta no SDK** — descartado (briefing §38/§40): primeiro eventos internos + atribuição + LP + Lead + Sessão; plataformas externas consomem essa camada depois.
- **Reaproveitar a fila offline criptografada (PA-05) no SDK** — descartado: ela é do usuário autenticado (chave derivada do `userId`); na LP não há sessão. Buffer em memória + `keepalive` cobre o caso real (fechar a aba logo após o clique).

**Impactos:** Aditivo. `leads` ganha 2 colunas nullable e 1 valor novo de `origem`; `fetchAll` faz 1 query a mais (tabela pequena); `fetchLeadsQrCode` (contexto "Captação" do vendedor) passa a incluir `landing_page` — leads de LP já distribuídos aparecem pro vendedor como os do Simulador. Nenhum fluxo existente de lead, RLS (`leads_select`/V-01) ou tela de vendedor/comercial alterados. Deploy exige: migração SQL, publicar 2 Edge Functions, incluir o domínio da LP em `CORS_ALLOWED_ORIGINS`, colar o snippet na LP (`doc/aquisicao/INTEGRACAO_LP.md`).

**Arquivos Afetados:** novos — `supabase/migracao-landing-pages.sql`, `supabase/functions/rastrear-lp/index.ts`, `supabase/functions/submeter-lp/index.ts`, `public/rjnet-lp.js`, `src/lib/aquisicao.js`, `src/api/landingPageApi.js`, `src/hooks/useAquisicaoMetricas.js`, `src/features/aquisicao/*` (AquisicaoTab, AquisicaoDashboard, AquisicaoFunil, AquisicaoFiltros, LandingPagesTab, LandingPageForm, LandingPageDetail, CampanhasTab, ConversoesTab), `tests/aquisicao.unit.test.js`, `tests/aquisicao.test.js`, `doc/aquisicao/AQUISICAO_ANALISE.md`, `doc/aquisicao/INTEGRACAO_LP.md`; alterados — `src/lib/dataService.js`, `src/context/AppProvider.jsx`, `src/apps/MarketingApp.jsx`, `src/components/ui.jsx`, `src/features/leads/LeadsTab.jsx`, `src/index.css`, `vercel.json`, `package.json`, docs.

**Riscos:** Médio-baixo. (1) Migração manual fora de ordem (dívida D-078) — arquivo idempotente e autocontido. (2) Ad-blockers subcontam métricas — documentado; conversão não depende do SDK. (3) Drift das Edge Functions no painel — checklist no guia. (4) `lp_events` cresce — retenção 395 dias + teto por sessão + limite por request. Validado com `node tests/aquisicao.unit.test.js` (32 asserções) e `npx playwright test tests/aquisicao.test.js` (tela do marketing em modo local + SDK numa LP sintética com Edge Functions interceptadas).

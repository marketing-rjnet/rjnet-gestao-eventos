...
`leads.perfil_consumo` grava um SNAPSHOT do que decidiu o lead (perguntas+respostas dos dois tipos, `perfil`/`combo` só em `oferta`) — não uma referência à campanha, que pode mudar depois; `resumoPerfil()` renderiza os blocos de forma independente (um lead só carrega o que é relevante ao seu tipo). Ver `doc/simulador/SIMULADOR_IMPLEMENTATION_PLAN.md`.

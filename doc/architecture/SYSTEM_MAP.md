...
### Simulador (D-072, D-076, D-077, D-080, D-081, D-082)
...
- **`quiz`** (D-080): teste de CONHECIMENTO — perguntas sempre de escolha única com uma `respostaCorretaId`, **configuráveis por campanha** (`simuladores.quiz_perguntas`, jsonb, construtor `QuizBuilder`) → pontuação = **contagem de acertos** (nunca peso/percentual, diferente de `demanda`) → **faixa de classificação** totalmente editável (`simuladores.quiz_faixas` — min/max de acertos, emoji, título) → tela de resultado com chamada de sorteio ("🎁 Deixe seu contato e concorra a um brinde RJNET!") → contato. Já nasce semeada com um molde de exemplo (`quizPerguntasPadrao()`/`quizFaixasPadrao()`, 10 perguntas de trivia motociclista + 4 faixas), primeiro uso real no evento MotoFest. Ganha também um **Sorteador** (dentro de `SimuladorTab.jsx`) que sorteia N ganhadores aleatoriamente entre TODOS os leads daquela campanha (`fetchLeadsPorSimulador`), independente de já distribuídos a vendedor.
...
**Resumo compartilhável do Quiz (D-082):** só no tipo `quiz`, na tela final (depois do contato já enviado) — card 1080x1080 desenhado num `<canvas>` (`desenharResumoQuiz()`, sem lib nova, mesma técnica do QR Code) com logo, nome da campanha, faixa (emoji+título), "{primeiro nome} acertou X de Y perguntas!", tempo de resposta (cronômetro `inicioQuizRef`/`tempoQuizMs`, só da 1ª à última pergunta, nunca enviado ao servidor) e o link `/s/:slug` no rodapé. Um único botão adapta-se ao navegador: `navigator.share({ files })` quando suporta compartilhar arquivo (abre o menu nativo do celular), ou "Baixar imagem" via `<a download>` quando não. Sem coluna nova, sem Edge Function tocada — puramente client-side.
...
- **Hardening de segurança pós-auditoria (D-078)**: consolidação de correções de uma auditoria completa (Pentest + Code Review), toda preservando o comportamento funcional. Regras que passam a valer:
  ...
  - **Dívida estrutural conhecida:** as migrações são coladas manualmente no SQL Editor (ambiguidade de ordem = causa raiz do V-01) e as Edge Functions no painel são cópias inline divergentes do `_shared` do repositório (drift). Recomendação de médio prazo acordada: adotar **Supabase CLI** com migração versionada.

...
### Simulador (D-072, D-076, D-077, D-080, D-081, D-082, D-083)
...
- **Retomada por `localStorage`** (chave por slug, só mesmo navegador): guarda `leadId`, pergunta atual, respostas e status (`cadastrado`/`concluido`). Sair e voltar retoma de onde parou sem recadastro; já ter concluído mostra tela de "já participou" em vez de permitir refazer.
...
- **Quiz de Acertos — cadastro ANTES do quiz, 2 fases na Edge Function, sem coluna nova (D-083)**: único tipo do Simulador que INVERTE a ordem "valor antes do dado" — o cadastro (nome/WhatsApp/LGPD) é gravado (INSERT, `fase:'cadastro'`) antes da 1ª pergunta, garantindo o lead mesmo com abandono no meio do quiz. `submeter-simulador` só aceita `body.fase` (`'cadastro'`/`'conclusao'`) pro tipo `quiz`; `oferta`/`demanda` nunca mandam esse campo e continuam com o insert único de sempre — mudar o comportamento de um não pode vazar pro outro. A conclusão (`fase:'conclusao'`) é um `UPDATE ... WHERE id=leadId AND simulador_id=X AND pontuacao is null` — a cláusula `pontuacao is null` é a própria guarda de atomicidade (1 chance só) E o sinal de "cadastro incompleto" usado em outros lugares (Sorteador), sem precisar de coluna/flag nova. Rate limit (D-067) só conta a fase `cadastro` (única que insere linha); `conclusao` não é limitada por IP porque sua única superfície de abuso é adivinhar um `leadId` (UUID de 128 bits gerado no servidor) — impraticável. `fetchLeadsPorSimulador` (`dataService.js`) e o filtro local do Sorteador (`SimuladorTab.jsx`) exigem `pontuacao != null` — só quem concluiu o quiz é sorteável, cadastro incompleto vira lead de CRM normal mas não entra na lista. Progresso/status (`localStorage`, chave por slug) permite retomar no mesmo navegador ou bloquear reentrada de quem já concluiu — mesmo navegador só, sem token cross-device.

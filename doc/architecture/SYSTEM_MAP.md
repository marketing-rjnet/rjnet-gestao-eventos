...
### Simulador de Perfil de Consumo (D-072)
Terceira porta pública de captação — quiz gamificado ("valor antes do dado": a pessoa responde 4–6 perguntas, recebe uma recomendação personalizada e só então deixa nome/WhatsApp/bairro/cidade). Cada linha de `simuladores` é uma **campanha** (nome, slug, agrupador) com link próprio (`/s/:slug`) pra tráfego pago e QR Code pra material impresso — o QR embute `utm_source=qrcode&utm_medium=impresso`, então **uma campanha só** distingue os dois canais via `leads.utm`. O questionário é catálogo **fixo** versionado em `src/lib/simulador.js` (`PERGUNTAS_SIMULADOR` — mesmo princípio do D-062, nunca motor de quiz genérico). A Edge Function `submeter-simulador` valida as respostas contra o catálogo e **recalcula no servidor** pontuação/temperatura/oferta (o cliente nunca manda score pronto); o lead nasce com `origem='simulador'`, `perfil_consumo` (jsonb `{versao, respostas}`), `pontuacao`, `oferta_recomendada`, `cidade`/`bairro`, `utm` e `vendedor_id` nulo — cai na mesma fila de distribuição do Form Builder, agora **ordenada por pontuação**. Gestão em `SimuladorTab.jsx` (grupo Captação do "Mais"), com seletor de tipo na criação. **Tipo `territorial` (D-073):** campanhas geolocalizadas com questionário reduzido — cidade* + bairro* + interesse*, sem quiz nem score (`temperatura='morno'` fixa, `pontuacao`/`perfil_consumo` nulos); alimenta o relatório interno **"Demanda por região"** em Relatórios (`LeadsTab.jsx`) via RPC `demanda_por_regiao()` (`migracao-demanda.sql`, só `cidade/bairro/count`, security definer, grant `authenticated` — nenhum dado pessoal sai da função, nunca expõe cobertura de rede). Ver `doc/simulador/SIMULADOR_IMPLEMENTATION_PLAN.md`.
...
- **Pacote fixo por perfil de uso + combo de upsell (D-074)**: no tipo `perfil_consumo`, a primeira etapa do wizard passou a ser uma pergunta de perfil (`PERFIS_SIMULADOR` — Básico/Streaming/Home Office/Gamer, cada um com pacote de internet FIXO associado, nunca calculado por soma de sinais). As perguntas antigas (moradores/usos/equipamentos/dificuldade) continuam existindo, mas alimentam só `pontuacao`/`temperatura` da fila — nunca o pacote. A tela de resultado monta um combo de checkboxes (Apps Yellow +R$15, Apps Black +R$30, upgrade pro próximo pacote) via `montarCombo()`, sempre recalculado a partir de `PACOTES_INTERNET`/`APPS_ADICIONAIS` (mesmo catálogo compartilhado com a aba "Pacotes" do vendedor, `VendedorApp.jsx` — preço único, sem duplicação) — nunca aceita um total pronto do cliente, mesmo princípio do score. Apps Black ganha destaque visual (não pré-marcado) quando `usos` inclui `streaming`. Gravado em `leads.perfil_consumo.perfil`/`.combo` (mesmo jsonb do D-072, sem migração nova).

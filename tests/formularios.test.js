...
test.describe('Validação de Formulários', () => {
  ...
  test('preencher e salvar evento cria novo card', async ({ page }) => {
    ...
    if (await dateInputs.count() > 0) await dateInputs.first().fill('2025-12-01');
    if (await dateInputs.count() > 1) await dateInputs.nth(1).fill('2025-12-02');
  ...
  test('lead salvo aparece na lista de leads do evento', async ({ page }) => {
    test.slow();
    await loginComercial(page);
    await page.locator('.event-card').first().click();
    await expect(page.locator('.detalhe-header')).toBeVisible();
    ...
    await leadForm.locator('input[type="text"]').first().fill('Lead Salvo E2E');

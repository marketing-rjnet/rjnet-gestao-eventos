...
```
Progresso geral: 3/18 etapas concluídas (17%)
```

```
Observações:
- Etapas 1 e 2 estavam marcadas como concluídas no documento anterior, mas o código
  não refletia isso. Foram executadas agora junto com a etapa 3.
- Build validado após cada etapa (vite build sem erros).
- Commits independentes por módulo criado + 1 commit para main.jsx.
```

```
Etapa 1  - ✅ Concluída — src/lib/formatUtils.js (fmtDate, fmtDateLong, initials)
Etapa 2  - ✅ Concluída — src/lib/masks.js (maskCpf, maskTel, validarCpf, validarTelefone)
Etapa 3  - ✅ Concluída — src/lib/csvUtils.js (buildLeadsCSV, downloadCSV)

...
Arquivo principal: src/main.jsx — 2.354 linhas (meta: < 100 linhas ao fim)
```
...
Etapa 6  — UI Components             ⬜ Não iniciada
...
| Arquivo | Responsabilidade |
|---------|-----------------|
| `src/components/ui/Icon.jsx` | Sistema de ícones SVG inline com 20+ ícones |
| `src/components/ui/ChartView.jsx` | Wrapper Chart.js com cleanup de instância no unmount |
| `src/components/ui/Badge.jsx` | `StatusBadge` e `TipoBadge` — badges de status/tipo |
| `src/components/ui/Kpi.jsx` | Card de KPI com label, valor, ícone e alerta opcional |
| `src/components/ui/index.js` | Re-exporta todos os componentes UI para import único |

### Arquivos modificados

| Arquivo | O que muda |
|---------|-----------|
| `src/main.jsx` | **Removido:** definições de `Icon`, `ChartView`, `StatusBadge`, `TipoBadge`, `Kpi` (linhas ~20–62, ~328–345, ~667–679). **Adicionado:** `import { Icon, ChartView, StatusBadge, TipoBadge, Kpi } from './components/ui'`. **Permanece:** toda a lógica de negócio, contexto e demais componentes. |
...
- Todos os 5 arquivos criados existem com os exports corretos.
- `src/main.jsx` não contém mais as definições extraídas.
- `src/main.jsx` importa via `./components/ui`.
- `npm run build` sem erros.

### Checklist de validação

- [ ] Ícones aparecem corretamente em toda a UI
- [ ] Gráfico de leads no Dashboard renderiza e destrói sem memory leak
- [ ] Gráfico de leads por evento em LeadsTab renderiza
- [ ] Badges de status aparecem na lista de eventos
- [ ] Badges de tipo aparecem na lista de eventos
- [ ] Cards de KPI exibem valores corretos no Dashboard
- [ ] Alerta visual no KPI de estoque crítico funciona

### Riscos conhecidos

- **Chart.js cleanup:** garantir que `useEffect` com `chart.destroy()` seja preservado em `ChartView.jsx` — memory leak se omitido.
- `Icon` é usado em praticamente todos os componentes; um import errado quebra a UI inteira.

### Resultado esperado

- ~100 linhas removidas de `main.jsx`.
- 5 novos arquivos totalizando ~120 linhas.
...
**→ Etapa 6 — UI Components** (primeira etapa não concluída)

**Resumo do que fazer:**

1. Ler `src/main.jsx` nas linhas ~20–62 (`Icon`), ~328–345 (`ChartView`), ~667–679 (`StatusBadge`, `TipoBadge`, `Kpi`).
2. Criar `src/components/ui/Icon.jsx`, `ChartView.jsx`, `Badge.jsx`, `Kpi.jsx`, `index.js`.
3. Remover as definições de `main.jsx`.
4. Adicionar import `import { Icon, ChartView, StatusBadge, TipoBadge, Kpi } from './components/ui'` em `main.jsx`.
5. Executar `npm run build` e verificar sem erros.
6. Testar os 7 itens do checklist de validação da Etapa 6.
7. Commit: `refactor: extract UI components (Icon, ChartView, Badge, Kpi)`.
